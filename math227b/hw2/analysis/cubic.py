...
def interpolation_results(f, a, b, n, x_test, bc_type="natural"):
    ...
    spline = cubic.interpolate(x_nodes, y_nodes, bc_type=bc_type)
    ...
    rms_error = np.sqrt(np.mean(np.power(errors, 2)))
...
def analyze_error(f, a, b, func_name, bc_type="natural"):
	n_values = list(range(5, 80, 3))  # number of spline nodes
	num_test_points = 500
	...
	for n in n_values:
		...
		rms_errors_f1.append(np.sqrt(np.mean(np.power(err1, 2))))
		...
		rms_errors_f2.append(np.sqrt(np.mean(np.power(err2, 2))))
	...
	ax1.loglog(hs, rms_errors_f, 'o--', label='RMS f error', color='C0')  # dashed, same color
	ax1.loglog(hs, max_errors_f[-1]*(hs/hs[-1])**4, 'k--', label='slope=4 ref')
	ax1.set_xlabel('h'); ax1.set_ylabel('Error'); ax1.set_title(f'f error'); ax1.grid(True); ax1.legend()
	...
	ax2.set_xlabel('h'); ax2.set_ylabel('Error'); ax2.set_title("f' error"); ax2.grid(True); ax2.legend()
	...
	ax3.set_xlabel('h'); ax3.set_ylabel('Error'); ax3.set_title("f'' error"); ax3.grid(True); ax3.legend()
